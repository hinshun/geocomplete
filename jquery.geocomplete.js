...
(function($, window, document, undefined){
  ...
  $.extend(GeoComplete.prototype, {
    ...
    initGeocoder: function () {
      ...
      this.$input.bind('keyup.' + this._name, $.proxy(function (e) {
        // Not an arrows
        if ([37, 38, 39, 40].indexOf(e.keyCode) == -1) {
          setTimeout($.proxy(this.saveLastValue, this), 0);
        }
      }, this));
    ...
    update: function(result){
      if (this.valid_result(result)) {
        ...
        this.trigger("geocode:filtered", result);
        setTimeout($.proxy(this.restoreLastValue, this), 0);
      }
    },
    ...
    parse_address_components: function(address_components) {
      var result_data = {};
      $.each(address_components, function(index, object){
        $.each(object.types, function(index, name){
          result_data[name] = object.long_name;
          result_data[name + "_short"] = object.short_name;
        });
      });
      return result_data;
    },
    ...
    valid_result: function(result){
      ...
      var valid = true;
      var result_data = this.parse_address_components(result.address_components);
      $.each(this.options.filters, function(field, allowed_values){
        if (allowed_values.indexOf(result_data[field]) === -1) {
          valid = false;
        }
      });
      return valid;
    ...
    fillDetails: function(result){
      ...
      var address_data = this.parse_address_components(result.address_components);
